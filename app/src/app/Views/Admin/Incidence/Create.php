    ...
    <form action="/path/to/your/controller" method="POST" enctype="multipart/form-data">
        <div class="mb-6">
            ...
            <select id="element_id" name="element_id"
                class="shadow appearance-none border rounded w-full py-3 px-4 text-gray-700 leading-tight focus:outline-none focus:ring focus:border-blue-500"
                required>
                <option value="" disabled selected>Selecciona un elemento</option>
                <?php foreach ($elements as $element): ?>
                    <?= '<option value="' . $element->getId() . '">' . $element->name . '</option>'; ?>
                <?php endforeach; ?>
            </select>
        </div>
        ...
        <div class="mb-6">
            ...
            <input type="text" id="name" name="name"
                class="shadow appearance-none border rounded w-full py-3 px-4 text-gray-700 leading-tight focus:outline-none focus:ring focus:border-blue-500"
                placeholder="Ejemplo: Rama caída" required>
        </div>
        ...
        <div class="mb-6">
            ...
            <textarea id="description" name="description"
                class="shadow appearance-none border rounded w-full py-3 px-4 text-gray-700 leading-tight focus:outline-none focus:ring focus:border-blue-500"
                placeholder="Describe la incidencia" rows="6" required></textarea>
        </div>
