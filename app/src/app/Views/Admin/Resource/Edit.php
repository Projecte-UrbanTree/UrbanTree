...
<div class="bg-white p-8 border border-gray-300 rounded-lg shadow-md">
    ...
    <form id="resourceForm" action="/admin/resource/<?= htmlspecialchars($resource->getId()); ?>/update" method="POST" class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div>
            ...
            <select id="category" name="type_resource_id" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring focus:ring-blue-500 focus:border-blue-500">
                <option value="" disabled selected>Seleccione una categoría</option>
                <?php foreach ($resource_types as $type_resource) { ?>
                    <option value="<?= htmlspecialchars($type_resource->getId()); ?>" <?= $type_resource->getId() == $resource->type_resource_id ? 'selected' : ''; ?>>
