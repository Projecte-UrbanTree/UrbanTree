...
<div class="bg-white p-8 border border-gray-300 rounded-lg shadow-md">
    ...
    <form id="workOrderForm" action="/admin/work-order/<?= htmlspecialchars($work_order->getId()); ?>/update"
        ...
        <div>
            ...
            <input type="text" name="userNames" id="workersInput" readonly
                ...
                value="<?= htmlspecialchars(implode(', ', array_map(fn($user) => $user->name . ' ' . $user->surname, $work_order->users()))); ?>"
            ...
            <input type="hidden" name="userIds" id="userIdsInput"
                value="<?= htmlspecialchars(implode(',', array_map(fn($user) => $user->getId(), $work_order->users()))); ?>">
        </div>
        ...
        <div id="blocksContainer" class="space-y-6">
            <?php foreach ($work_order->blocks() as $blockIndex => $block) { ?>
                <div class="workorder-block border border-gray-300 rounded-lg shadow p-4 bg-gray-50 mb-4"
                    ...
                    <div>
                        <label for="zonesInput_<?= $blockIndex ?>"
                            class="block text-sm font-medium text-gray-700 mb-1">Zonas</label>
                        <input type="text" id="zonesInput_<?= $blockIndex ?>" readonly
                            onclick="openModal('modalZones', 'zonesInput_<?= $blockIndex ?>')"
                            placeholder="Seleccionar Zonas"
                            value="<?= htmlspecialchars(implode(', ', array_map(fn($zone) => $zone->name, $block->zones()))); ?>"
                            class="w-full px-4 py-2 border border-gray-300 rounded-lg bg-gray-100 cursor-pointer focus:outline-none">
                        <input type="hidden" name="blocks[<?= $blockIndex ?>][zonesIds]"
                            id="zonesIdsInput_<?= $blockIndex ?>"
                            value="<?= htmlspecialchars(implode(',', array_map(fn($zone) => $zone->getId(), $block->zones()))); ?>">
                    </div>
                    <div class="tasksContainer space-y-4">
                        ...
                        <?php foreach ($block->tasks() as $taskIndex => $task) { ?>
                            <div class="task-row flex space-x-4 items-end" data-task-index="<?= $taskIndex ?>">
                                <div class="flex-auto">
                                    <select name="blocks[<?= $blockIndex ?>][tasks][<?= $taskIndex ?>][taskType]"
                                        ...
                                        <?php foreach ($task_types as $task_type) { ?>
                                            <option value="<?= htmlspecialchars($task_type->getId()) ?>"
                                                <?= $task_type->getId() == $task->task_id ? 'selected' : '' ?>>
                                ...
                                <div class="flex-auto">
                                    <select name="blocks[<?= $blockIndex ?>][tasks][<?= $taskIndex ?>][elementType]"
                                        id="elementType_<?= $blockIndex ?>_<?= $taskIndex ?>"
                                        class="w-full px-4 py-2 border border-gray-300 rounded-lg bg-white focus:outline-none focus:ring focus:ring-blue-500 focus:border-blue-500">
                                        <option value="">Seleccione un tipo de elemento</option>
                                        <?php foreach ($element_types as $element_type) { ?>
                                            <option value="<?= htmlspecialchars($element_type->getId()) ?>"
                                                <?= $element_type->getId() == $task->element_type_id ? 'selected' : '' ?>>
                                ...
                                <div class="flex-auto flex items-center space-x-2">
                                    ...
                                    <select name="blocks[<?= $blockIndex ?>][tasks][<?= $taskIndex ?>][species]"
                                        ...
                                        <?php foreach ($tree_types as $tree) { ?>
                                            <option value="<?= htmlspecialchars($tree->getId()) ?>"
                                                <?= $tree->getId() == $task->tree_type_id ? 'selected' : '' ?>>
                    ...
                    <div class="mt-4">
                        <label for="notes_<?= $blockIndex ?>"
                            class="block text-sm font-medium text-gray-700 mb-1">Notas</label>
                        <textarea name="blocks[<?= $blockIndex ?>][notes]" id="notes_<?= $blockIndex ?>" rows="4"
                            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring focus:ring-blue-500 focus:border-blue-500 resize-none"><?= htmlspecialchars($block->notes); ?></textarea>
                    </div>
                </div>
            <?php } ?>
        </div>
